...
GROUP BY 
    productCode
HAVING 
    SUM(quantityOrdered) > 0   -- optional filter to exclude products with no orders
ORDER BY 
    totalQuantity DESC
LIMIT 10;
...
SELECT 
    ...
    COUNT(DISTINCT checknumber) AS num_payment
FROM Payments
GROUP BY MONTHNAME(paymentdate)
HAVING COUNT(DISTINCT checknumber) > 20
...
-- a. Create table facility with initial fields
...
-- i) Alter the table to add primary key and auto_increment to Facility_ID
ALTER TABLE facility
MODIFY Facility_ID INT NOT NULL AUTO_INCREMENT,
ADD PRIMARY KEY (Facility_ID);

-- ii) Add a new column 'City' after 'Name' with NOT NULL constraint
ALTER TABLE facility
...
describe product_category_sales;
...
BEGIN
    SELECT 
        ...
        CONCAT(ROUND(SUM(p.amount) / 1000), 'K') AS `Total Amount`
    FROM 
        payments p
    JOIN 
...
SELECT
  ...
  CASE
    WHEN pm.total_orders IS NULL OR pm.total_orders = 0 THEN NULL
    ELSE CONCAT(
      ROUND(((m.total_orders - pm.total_orders) / pm.total_orders) * 100), '%'
    )
  END AS `% Change vs Prev Month`
FROM (
  SELECT
    YEAR(orderdate) AS yr,
    MONTH(orderdate) AS mnum,
...
LEFT JOIN (
  ...
  ON pm.yr = m.yr
 AND pm.mnum = m.mnum - 1
ORDER BY m.yr, m.mnum;
...
BEGIN
    -- Exit handler: stops execution on error
